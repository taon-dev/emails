    ...
    <taon-admin-mode-configuration>
      @if (itemsLoaded()) {
        ...
        <footer
          ...
          {{ t.gettext('Copyright') }} <strong>emails</strong>
          {{ year }}
        </footer>
      }
    </taon-admin-mode-configuration>
...
export class EmailsApp implements OnInit {
  t = t.for(this);
  ...
  navItems =
    EmailsClientRoutes.length <= 1
      ? []
      : EmailsClientRoutes.filter(r => r.path !== undefined).map(r => ({
          path: r.path === '' ? '/' : `/${r.path}`,
...
//#region  emails routes
//#region @browser
export const EmailsServerRoutes: ServerRoute[] = [
  {
    path: '**',
    renderMode: RenderMode.Prerender,
  },
];
export const EmailsClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (EmailsClientRoutes.length === 1) {
        return '';
      }
      return EmailsClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
//#region  emails app configs
//#region @browser
export const EmailsAppConfig: ApplicationConfig = {
  providers: [
    provideZonelessChangeDetection(),
    {
      provide: TAON_CONTEXT,
      useFactory: () => EmailsContext,
    },
    providePrimeNG({
      theme: {
        preset: Aura,
    ...
    }),
    {
      ...
      multi: true,
      useFactory: () => EmailsStartFunction,
    },
    provideBrowserGlobalErrorListeners(),
...
export const EmailsServerConfig: ApplicationConfig = {
  providers: [provideServerRendering(withRoutes(EmailsServerRoutes))],
};

export const EmailsConfig = mergeApplicationConfig(
  EmailsAppConfig,
  EmailsServerConfig,
);
//#endregion
...
//#region  emails context
var EmailsContext = Taon.createContext(() => ({
  ...HOST_CONFIG['EmailsContext'],
...
//#region  emails start function
export const EmailsStartFunction = async (
  ...
  await EmailsContext.initialize(startParams);
...
export default EmailsStartFunction;
//#endregion
